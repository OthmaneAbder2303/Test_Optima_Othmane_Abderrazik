...
**Tentative avec spaCy (fr_core_news_md)** : j'ai testé une approche basée sur la similarité vectorielle de spaCy **(doc.similarity())**. Les scores étaient globalement cohérents, mais le système produisait des incohérences de preuve - il sélectionnait une preuve avec un bon score cosinus qui ne correspondait pas sémantiquement à l'exigence. Exemple concret : pour REQ-07 ("mentionner explicitement les risques résiduels identifiés"), la meilleure preuve retournée était "Evaluation des risques : REALISEE pour la phase d'utilisation uniquement" `(sim=0.78)` au lieu de "Contenu notice - risques résiduels : OUI, section 8.3". Le modèle rapprochait "risques" dans les deux textes sans distinguer l'évaluation des risques de la mention des risques résiduels dans la notice. Le verdict était parfois correct par accident, mais la justification était fausse - ce qui est rédhibitoire dans un contexte de conformité réglementaire où la traçabilité de la décision est aussi importante que le verdict lui-même. Le scoring lexical pondéré sur la clé de champ `(overlap_clé × 2)` s'est révélé plus fiable car il ancre la recherche sur le champ sémantique exact, pas sur une similarité globale inter-phrases.
